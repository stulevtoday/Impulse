...
import { computeFileComplexity, type FunctionComplexity, type ComplexityRisk } from "../core/complexity.js";

export function registerFocusCommand(program: Command): void {
  program
    .command("focus")
    .description("Deep analysis of a single file — imports, exports, impact, complexity, tests, git")
    .argument("<file>", "Relative path to the file")
    .argument("[dir]", "Project root directory", ".")
    .option("--json", "Output as JSON")
    .action(async (file: string, dir: string, opts: { json?: boolean }) => {
      const rootDir = resolve(dir);
      const [{ graph, stats }, parsed] = await Promise.all([
        analyzeProject(rootDir),
        parseFile(rootDir, file),
      ]);
      const report = focusFile(graph, file, rootDir);
      const functions = parsed ? computeFileComplexity(parsed) : [];

      if (opts.json) {
        console.log(JSON.stringify({ ...report, complexity: functions }, null, 2));
        return;
      }
      ...
      for (const i of localImports) console.log(`    → ${i}`);
      if (externalImports.length > 0) console.log(`    ${dim}${externalImports.join(", ")}${reset}`);

      console.log(`\n  ${cyan}Imported by${reset}  ${bold}${report.importedBy.length}${reset} file(s)`);
      for (const f of report.importedBy.slice(0, 10)) console.log(`    ← ${f}`);
      if (report.importedBy.length > 10) console.log(`    ${dim}...and ${report.importedBy.length - 10} more${reset}`);
      ...
      if (report.exports.length > 0) {
        ...
        for (const e of alive) console.log(`    ${green}✓${reset} ${e.name}  ${dim}→ ${e.consumers.length} consumer(s)${reset}`);
        for (const e of dead) console.log(`    ${red}✗${reset} ${e.name}  ${dim}— unused${reset}`);
      }
      ...
      if (Object.keys(report.impactByDepth).length > 0) {
        ...
        for (let d = 1; d <= maxDepth; d++) {
          ...
          console.log(`    ${bar} ${count}  ${dim}(${d === 1 ? "direct" : `depth ${d}`})${reset}`);
        }
      }

      printComplexitySection(functions);

      console.log(`\n  ${cyan}Test coverage${reset}  ${report.testsCovering.length > 0 ? `${green}${report.testsCovering.length} test(s)${reset}` : `${red}no tests${reset}`}`);
      for (const t of report.testsCovering) console.log(`    ${green}⚡${reset} ${t}`);
      ...
      if (report.topCochangers.length > 0) {
        ...
        for (const c of report.topCochangers) console.log(`    ${c.file}  ${dim}(${c.cochanges}×)${reset}`);
      }
...
const RISK_COLORS: Record<ComplexityRisk, string> = {
  simple: "\x1b[32m", moderate: "\x1b[36m", complex: "\x1b[33m", alarming: "\x1b[31m",
};

function printComplexitySection(functions: FunctionComplexity[]): void {
  const dim = "\x1b[2m";
  const reset = "\x1b[0m";
  const bold = "\x1b[1m";
  const cyan = "\x1b[36m";
  ...
  const sorted = [...functions].sort((a, b) => b.cognitive - a.cognitive);
  const maxCog = sorted.length > 0 ? Math.max(...sorted.map((f) => f.cognitive)) : 0;

  console.log(`\n  ${cyan}Complexity${reset}  ${bold}${functions.length}${reset} function(s), max cognitive ${bold}${maxCog}${reset}`);

  for (const fn of sorted.slice(0, 8)) {
    const color = RISK_COLORS[fn.risk];
    const label = fn.risk === "simple" ? "" : ` ${color}${fn.risk.toUpperCase()}${reset}`;
    console.log(`    ${fn.name}  ${dim}cyc ${fn.cyclomatic} · cog ${fn.cognitive}${reset}${label}`);
  }
  if (sorted.length > 8) {
    console.log(`    ${dim}...and ${sorted.length - 8} more${reset}`);
  }
}
